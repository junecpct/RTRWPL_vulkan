...
//--------------------------------------------------------------------------------------------------
// Creating receive sphere (nbSpheres = 1)
//
void HelloVulkan::createSpheres(uint32_t nbSpheres)
{
  ...
  {
    Sphere s;
    s.center     = glm::vec3(-55.19f, 71.0f, -62.049f);
    s.radius     = 0.04f;
...
void HelloVulkan::createRtPipeline()
{
  ...
  stage.module = nvvk::createShaderModule(m_device, nvh::loadFile("spv/triangle.rchit.spv", true, defaultSearchPaths, true));
  stage.stage         = VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR;
  stages[eClosestHit] = stage;
  // Closest hit
  stage.module = nvvk::createShaderModule(m_device, nvh::loadFile("spv/rcvsphere.rchit.spv", true, defaultSearchPaths, true));
  stage.stage          = VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR;
  stages[eClosestHit2] = stage;
  // Intersection
  stage.module = nvvk::createShaderModule(m_device, nvh::loadFile("spv/rcvsphere.rint.spv", true, defaultSearchPaths, true));
...
void HelloVulkan::raytrace(const VkCommandBuffer& cmdBuf, const glm::vec4& clearColor)
{
  ...
  m_pcRay.totalWidth     = SAMPLE_WIDTH;
  m_pcRay.totalHeight    = SAMPLE_HEIGHT;

  const uint32_t totalRaysToGenerate   = SAMPLE_WIDTH * SAMPLE_HEIGHT;     
  const uint32_t raysPerIteration      = FRAME_WIDTH * FRAME_HEIGHT;    
  uint32_t globalRayCounter = 0;


  while (globalRayCounter < totalRaysToGenerate)
  {  
    // Calculating the remaining rays & adjusting the chunk size
    uint32_t remainingRays = totalRaysToGenerate - globalRayCounter;
    uint32_t raysThisIteration = std::min(remainingRays, raysPerIteration);

    m_pcRay.raysThisIteration = raysThisIteration;
    
    m_pcRay.globalRayCounter = globalRayCounter;
    ...
    globalRayCounter += raysThisIteration;

    // Insert a pipeline barrier to synchronize before dispatching the next set of rays
    VkMemoryBarrier memoryBarrier = {
      VK_STRUCTURE_TYPE_MEMORY_BARRIER,
      nullptr,
      VK_ACCESS_SHADER_WRITE_BIT,  // Specify the access mask for the stage whose execution is finishing
      VK_ACCESS_SHADER_READ_BIT    // Specify the access mask for the stage whose execution is starting
    };
